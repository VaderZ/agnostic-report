...
from .. import schemas
from ...db.dal import get_reporting, Reporting

router = APIRouter(prefix='/reporting', tags=['Reporting'])


@router.get('/projects', response_model=schemas.PagedProjects)
async def get_projects(reporting: Reporting = Depends(get_reporting),
...
async def get_tests_over_time(project_id: UUID,
                              reporting: Reporting = Depends(get_reporting),
    ...
    return await reporting.get_test_runs(project_id, order_by, order, page, page_size,
...
async def get_test_run_filters(project_id: UUID, reporting: Reporting = Depends(get_reporting),
                               interval: str | None = Query(None)):
...
async def get_test_run_tests(project_id: UUID,
                             ...
                             page_size: int = Query(10, ge=1),
                             reporting: Reporting = Depends(get_reporting)):
    return await reporting.get_tests(project_id, test_run_id, result, search, order, order_by, page, page_size)
...
async def get_test_run_metrics(project_id: UUID,
                               ...
                               search: list[str] | None = Query(None),
                               reporting: Reporting = Depends(get_reporting)):
    return await reporting.get_test_run_metrics_and_properties(project_id, test_run_id, result, search, metrics)
