...
class Progress:
    ...
    async def get(self, id: UUID) -> schemas.v1.Progress:
        progress = (
        ...
        return schemas.v1.Progress.model_validate(progress)

    async def get_all(self, test_run_id: UUID | None = None, test_id: UUID | None = None) -> [schemas.v1.Progress]:
        ...
        return [schemas.v1.Progress.model_validate(progress) for progress in progresses]

    async def create(self, progress: schemas.v1.ProgressCreate) -> UUID:
    ...
    async def update(self, progress: schemas.v1.Progress, exclude_unset: bool = False) -> UUID:
        try:
            result = (
                await self.session.execute(
