...
class MetricsOverTime:
    ...
    async def get(self, id: UUID) -> schemas.v1.MetricOverTime:
        metric = (
        ...
        return schemas.v1.MetricOverTime.model_validate(metric)

    async def get_all(self, test_run_id: UUID | None = None, test_id: UUID | None = None) -> [schemas.v1.MetricOverTime]:
        ...
        return [schemas.v1.MetricOverTime.model_validate(metric) for metric in metrics]

    async def create(self, metric: schemas.v1.MetricOverTimeCreate) -> UUID:
    ...
    async def update(self, metric: schemas.v1.MetricOverTime, exclude_unset: bool = False) -> UUID:
        try:
            result = (
                await self.session.execute(
