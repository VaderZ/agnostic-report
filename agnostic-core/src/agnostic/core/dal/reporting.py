...
import agnostic.core.schemas.v1.reporting.logs
import agnostic.core.schemas.v1.reporting.metrics
import agnostic.core.schemas.v1.reporting.metrics_ot
import agnostic.core.schemas.v1.reporting.progress
import agnostic.core.schemas.v1.reporting.projects
import agnostic.core.schemas.v1.reporting.test_runs
import agnostic.core.schemas.v1.reporting.tests
import agnostic.core.schemas.v1.reporting.widgets
...
class Reporting:
    ...
    async def get_projects(self, order_by: str, order: str,
                           page: int, page_size: int) -> agnostic.core.schemas.v1.reporting.projects.PagedProjects:
        pages = (
        ...
        return agnostic.core.schemas.v1.reporting.projects.PagedProjects(
            data=[agnostic.core.schemas.v1.reporting.projects.ProjectStatistics.model_validate(project) for project in projects],
            count=pages.count,
            pages=pages.pages,
            page=page,
            page_size=page_size
    ...
    async def get_test_runs(self,
                            ...
                            variant: list[str] | None = None,
                            interval: str | None = None,
                            test_run_id: UUID | None = None) -> agnostic.core.schemas.v1.reporting.test_runs.PagedTestRuns:
        ...
        def make_test_run_statistics(test_run) -> agnostic.core.schemas.v1.reporting.test_runs.TestRunsStatistics:
            terminated = is_terminated(test_run)
            tr = agnostic.core.schemas.v1.reporting.test_runs.TestRunsStatistics.model_validate(test_run)
            status = agnostic.core.schemas.v1.reporting.test_runs.TestRunStatus(
        ...
        return agnostic.core.schemas.v1.reporting.test_runs.PagedTestRuns(
    ...
    async def get_tests_over_time(self, project_id: UUID,
                                  ...
                                  test_branch: list[str] | None = None,
                                  variant: list[str] | None = None,
                                  interval: str | None = None) -> agnostic.core.schemas.v1.reporting.widgets.TestsOverTime:
        ...
        by_date = select(
            ...
            ).filter(models.Test.result == schemas.v1.TestResult.PASSED).label('passed'),
            func.count(
                func.distinct(models.Test.id)
            ).filter(models.Test.result == schemas.v1.TestResult.FAILED).label('failed'),
            func.count(
                func.distinct(models.Test.id)
            ).filter(models.Test.result == schemas.v1.TestResult.XPASSED).label('xpassed'),
            func.count(
                func.distinct(models.Test.id)
            ).filter(models.Test.result == schemas.v1.TestResult.XFAILED).label('xfailed'),
            func.count(
                func.distinct(models.Test.id)
            ).filter(models.Test.result == schemas.v1.TestResult.SKIPPED).label('skipped'),
        ).join(
        ...
        return agnostic.core.schemas.v1.reporting.widgets.TestsOverTime(
            data=agnostic.core.schemas.v1.reporting.widgets.TestsOverTimeData(
                series=[agnostic.core.schemas.v1.reporting.widgets.TestsOverTimeSeries(name=name, data=getattr(by_date, name) or []) for name in
                        ['skipped', 'xfailed', 'xpassed', 'failed', 'passed']],
                categories=by_date.date or []
            )
        )

    async def get_test_run_filters(self, project_id: UUID, interval: str | None = None) -> agnostic.core.schemas.v1.reporting.test_runs.TestRunFilters:
        ...
        return agnostic.core.schemas.v1.reporting.test_runs.TestRunFilters(
            data=agnostic.core.schemas.v1.reporting.test_runs.TestRunFiltersData(
    ...
    async def get_top_failed_test(self,
                                  ...
                                  variant: list[str] | None = None,
                                  interval: str | None = None,
                                  limit: int | None = 5) -> agnostic.core.schemas.v1.reporting.widgets.TopFailedTests:
        ...
        stat = select(
            ...
            func.count(
                ...
                models.Test.result == schemas.v1.TestResult.FAILED
            ).label('failed')
        ).join(
        ...
        return agnostic.core.schemas.v1.reporting.widgets.TopFailedTests(
            data=[agnostic.core.schemas.v1.reporting.widgets.TopFailedTestsData.model_validate(test) for test in top_failed]
        )
    ...
    async def get_project_metrics(self,
                                  ...
                                  interval: str | None = None,
                                  metrics: list[
                                               agnostic.core.schemas.v1.reporting.metrics.MetricRequest] | None = None) -> agnostic.core.schemas.v1.reporting.metrics.MetricsAggregate:
        tr_filters, trunc_to = get_test_run_filter(project_id, sut_branch, test_branch, variant, interval)
        ...
        result = agnostic.core.schemas.v1.reporting.metrics.MetricsAggregate(data=[])
        for metric in metrics:
            if metric.table == 'metrics':
            ...
            result.data.append(
                agnostic.core.schemas.v1.reporting.metrics.MetricsData(
                    name=metric.title,
    ...
    async def get_tests_by_result(self,
                                  ...
                                  search: list[str] | None) -> agnostic.core.schemas.v1.reporting.widgets.TestsByResult:
        by_result = (
            await self.session.execute(
                select(
                    ...
                    case_(
                        {
                            schemas.v1.TestResult.PASSED: 1,
                            schemas.v1.TestResult.FAILED: 2,
                            schemas.v1.TestResult.XPASSED: 3,
                            schemas.v1.TestResult.XFAILED: 4,
                            schemas.v1.TestResult.SKIPPED: 5
                        },
        ...
        return agnostic.core.schemas.v1.reporting.widgets.TestsByResult(
            data=agnostic.core.schemas.v1.reporting.widgets.TestsByResultData(
    ...
    async def get_tests(self,
                        ...
                        order_by: str,
                        page: int,
                        page_size: int) -> agnostic.core.schemas.v1.reporting.tests.PagedTests:
        ...
        return agnostic.core.schemas.v1.reporting.tests.PagedTests(
            data=[agnostic.core.schemas.v1.reporting.tests.TestsStatistics.model_validate(test) for test in tests],
            count=pages.count,
            pages=pages.pages,
            page=page,
            page_size=page_size
    ...
    async def get_test_run_metrics_list(self,
                                        ...
                                        order_by: str,
                                        page: int,
                                        page_size: int) -> agnostic.core.schemas.v1.reporting.metrics.PagedTestRunMetricsList:
        pages = (
            await self.session.execute(
                select(
        ...
        data = []
        for metric in metrics:
            data.append(agnostic.core.schemas.v1.reporting.metrics.TestRunMetricsListStatistics(
                name=metric.name,
                value=metric.value,
                description=metric.description.format(metric.value)
            ))

        return agnostic.core.schemas.v1.reporting.metrics.PagedTestRunMetricsList(
            data=data,
            count=pages.count,
            pages=pages.pages,
            page=page,
    ...
    async def get_test_run_progress(self,
                                    ...
                                    order_by: str,
                                    page: int,
                                    page_size: int) -> agnostic.core.schemas.v1.reporting.progress.PagedTestRunProgressRecords:
        ...
        return agnostic.core.schemas.v1.reporting.progress.PagedTestRunProgressRecords(
            data=[agnostic.core.schemas.v1.reporting.progress.TestRunProgressRecord.model_validate(record) for record in progress],
            count=pages.count,
            pages=pages.pages,
            page=page,
            page_size=page_size
    ...
    async def get_test_run_logs(self,
                                ...
                                order_by: str,
                                page: int,
                                page_size: int) -> agnostic.core.schemas.v1.reporting.logs.PagedTestRunLog:
        pages = (
            await self.session.execute(
                select(
        ...
        return agnostic.core.schemas.v1.reporting.logs.PagedTestRunLog(
            data=[agnostic.core.schemas.v1.reporting.logs.TestRunLog.model_validate(log) for log in logs],
            count=pages.count,
            pages=pages.pages,
            page=page,
            page_size=page_size
    ...
    async def get_test_run_over_time_metric(self,
        ...
        if not name or not key:
            return agnostic.core.schemas.v1.reporting.metrics_ot.MetricOverTimeReport()
        ...
        return agnostic.core.schemas.v1.reporting.metrics_ot.MetricOverTimeReport(
            data=agnostic.core.schemas.v1.reporting.metrics_ot.MetricOverTimeData(
                series=[agnostic.core.schemas.v1.reporting.metrics_ot.MetricOverTimeSeries(name=k, data=getattr(series, k) or []) for k in key],
    ...
    async def get_test_details(self,
                               project_id: UUID,
                               test_run_id: UUID,
                               test_id: UUID) -> agnostic.core.schemas.v1.reporting.tests.TestReport:
        ...
        return agnostic.core.schemas.v1.reporting.tests.TestReport(
