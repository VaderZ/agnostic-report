...
class Metrics:
    ...
    async def get(self, id: UUID) -> schemas_v1.Metric:
        metric = (
        ...
        return schemas_v1.Metric.model_validate(metric)

    async def get_all(self, test_run_id: UUID | None = None, test_id: UUID | None = None) -> [schemas_v1.Metric]:
        ...
        return [schemas_v1.Metric.model_validate(metric) for metric in metrics]

    async def create(self, metric: schemas_v1.MetricCreate) -> UUID:
    ...
    async def update(self, metric: schemas_v1.Metric, exclude_unset: bool = False) -> UUID:
        try:
            result = (
                await self.session.execute(
