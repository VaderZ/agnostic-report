...
async def create_test_run(test_run: schemas.v1.TestRun, project_id: UUID,
...
async def update_test_run(test_run: schemas.v1.TestRun, project_id: UUID, test_run_id: UUID,
                          response: Response, test_runs: dal.TestRuns = Depends(dal.get_test_runs)):
...
async def update_test_run_fields(test_run: schemas.v1.TestRun, project_id: UUID, test_run_id: UUID,
...
async def start_test_run(project_id: UUID, test_run_id: UUID,
                         test_run: schemas.v1.TestRun, test_runs: dal.TestRuns = Depends(dal.get_test_runs)):
    try:
        await test_runs.update(
            schemas.v1.TestRun(id=test_run_id, start=test_run.start or datetime.datetime.utcnow()),
            exclude_unset=True
        )
    except dal.NotFoundError as e:
        raise HTTPException(
...
async def finish_test_run(project_id: UUID, test_run_id: UUID,
    ...
    try:
        await test_runs.update(
            schemas.v1.TestRun(id=test_run_id, finish=test_run.finish or datetime.datetime.utcnow()),
            exclude_unset=True
        )
    except dal.NotFoundError as e:
        raise HTTPException(
...
async def update_test_run_heartbeat(project_id: UUID, test_run_id: UUID,
    ...
    try:
        await test_runs.update(
            schemas.v1.TestRun(id=test_run_id, heartbeat=test_run.heartbeat or datetime.datetime.utcnow()),
            exclude_unset=True
        )
    except dal.NotFoundError as e:
        raise HTTPException(
...
async def add_test_run_property(project_id: UUID, test_run_id: UUID,
                                prop: schemas.v1.KeyValue, test_runs: dal.TestRuns = Depends(dal.get_test_runs)):
    try:
...
@router.get('/projects/{project_id}/test-runs/{test_run_id}', response_model=schemas.v1.TestRun)
...
@router.get('/projects/{project_id}/test-runs', response_model=list[schemas.v1.TestRun])
