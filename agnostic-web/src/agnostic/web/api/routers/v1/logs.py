...
@router.post(
    "/projects/{project_id}/test-runs/{test_run_id}/logs", status_code=status.HTTP_201_CREATED
)
async def create_test_run_log(
    log: schemas.v1.LogCreate,
    project_id: UUID,
    test_run_id: UUID,
    response: Response,
    ...
    try:
        ...
        response.headers.append(
            "Location", f"/projects/{project_id}/test-runs/{test_run_id}/logs/{log_id}"
        )
...
async def update_test_run_log_fields(
    log: schemas.v1.Log,
    project_id: UUID,
    test_run_id: UUID,
    log_id: UUID,
...
async def append_test_run_log(
    body: schemas.v1.StringValue,
    project_id: UUID,
    test_run_id: UUID,
    log_id: UUID,
    logs: dal.Logs = Depends(dal.get_logs),
...
@router.get(
    "/projects/{project_id}/test-runs/{test_run_id}/logs", response_model=list[schemas.v1.Log]
)
async def get_test_run_logs(
    project_id: UUID, test_run_id: UUID, logs: dal.Logs = Depends(dal.get_logs)
):
    return await logs.get_all(test_run_id)


@router.get(
    "/projects/{project_id}/test-runs/{test_run_id}/logs/{log_id}", response_model=schemas.v1.Log
)
async def get_test_run_log(
    project_id: UUID, test_run_id: UUID, log_id: UUID, logs: dal.Logs = Depends(dal.get_logs)
):
    try:
...
async def download_test_run_log(
    project_id: UUID, test_run_id: UUID, log_id: UUID, logs: dal.Logs = Depends(dal.get_logs)
):
    try:
...
@router.post(
    "/projects/{project_id}/test-runs/{test_run_id}/tests/{test_id}/logs",
...
async def update_test_log_fields(
    log: schemas.v1.Log,
    project_id: UUID,
    test_run_id: UUID,
    test_id: UUID,
...
@router.get(
    "/projects/{project_id}/test-runs/{test_run_id}/tests/{test_id}/logs/{log_id}",
    response_model=schemas.v1.Log,
)
...
@router.get(
    "/projects/{project_id}/test-runs/{test_run_id}/tests/{test_id}/logs",
    response_model=list[schemas.v1.Log],
)
async def get_test_logs(
    project_id: UUID, test_run_id: UUID, test_id: UUID, logs: dal.Logs = Depends(dal.get_logs)
):
