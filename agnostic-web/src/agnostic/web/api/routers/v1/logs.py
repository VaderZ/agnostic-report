...
async def create_test_run_log(log: schemas.v1.LogCreate, project_id: UUID, test_run_id: UUID,
...
async def update_test_run_log(log: schemas.v1.Log, project_id: UUID, test_run_id: UUID, log_id: UUID,
                              response: Response, logs: dal.Logs = Depends(dal.get_logs)):
    log.id = log_id
    log.test_run_id = test_run_id
    try:
        ...
        try:
            await logs.create(schemas.v1.LogCreate.parse_obj(log))
...
async def update_test_run_log_fields(log: schemas.v1.Log, project_id: UUID, test_run_id: UUID, log_id: UUID,
...
async def append_test_run_log(body: schemas.v1.StringValue, project_id: UUID, test_run_id: UUID, log_id: UUID,
                              logs: dal.Logs = Depends(dal.get_logs)):
    try:
...
@router.get('/projects/{project_id}/test-runs/{test_run_id}/logs', response_model=list[schemas.v1.Log])
...
@router.get('/projects/{project_id}/test-runs/{test_run_id}/logs/{log_id}', response_model=schemas.v1.Log)
...
async def create_test_log(log: schemas.v1.LogCreate, project_id: UUID, test_run_id: UUID, test_id: UUID,
                          response: Response, logs: dal.Logs = Depends(dal.get_logs)):
    log.test_run_id = test_run_id
    log.test_id = test_id
    try:
...
async def update_test_log(log: schemas.v1.Log, project_id: UUID, test_run_id: UUID, test_id: UUID, log_id: UUID,
                          response: Response, logs: dal.Logs = Depends(dal.get_logs)):
    log.id = log_id
    log.test_run_id = test_run_id
    log.test_id = test_id
...
async def update_test_log_fields(log: schemas.v1.Log, project_id: UUID, test_run_id: UUID, test_id: UUID, log_id: UUID,
                                 logs: dal.Logs = Depends(dal.get_logs)):
    log.id = log_id
    log.test_run_id = test_run_id
    log.test_id = test_id
...
async def append_test_log(body: schemas.v1.StringValue, project_id: UUID, test_run_id: UUID, log_id: UUID,
                          logs: dal.Logs = Depends(dal.get_logs)):
    try:
        await logs.append_body(log_id, body.value)
    except dal.NotFoundError as e:
...
@router.get('/projects/{project_id}/test-runs/{test_run_id}/tests/{test_id}/logs/{log_id}', response_model=schemas.v1.Log)
...
@router.get('/projects/{project_id}/test-runs/{test_run_id}/tests/{test_id}/logs', response_model=list[schemas.v1.Log])
