...
async def create_progress_record(progress: schemas.v1.ProgressCreate, project_id: UUID, test_run_id: UUID,
...
async def update_progress_record(progress: schemas.v1.Progress, project_id: UUID, test_run_id: UUID, record_id: UUID,
                                 response: Response, progresses: dal.Progress = Depends(dal.get_progress)):
    ...
    try:
        ...
        try:
            await progresses.create(schemas.v1.ProgressCreate.parse_obj(progress))
...
async def update_progress_record_fields(progress: schemas.v1.Progress, project_id: UUID, test_run_id: UUID, record_id: UUID,
...
@router.get('/projects/{project_id}/test-runs/{test_run_id}/progress/{record_id}', response_model=schemas.v1.Progress)
...
@router.get('/projects/{project_id}/test-runs/{test_run_id}/progress', response_model=list[schemas.v1.Progress])
